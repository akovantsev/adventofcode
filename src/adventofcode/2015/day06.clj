...
(defn parse-line [s]
  (let [[_ op x1 y1 x2 y2] (re-matches #"([a-z ]+) (\d+),(\d+) through (\d+),(\d+)" s)]
...
(defn set-points [fns !m {:keys [x1 x2 y1 y2 op]}]
  (reduce
    (fn rx [!m x]
      (reduce
        (fn ry [!m y]
...
(def init-op {:x1 0 :x2 999 :y1 0 :y2 999 :op :off})
...
(defn f1 [ops]
  (let [fns {:t   not
    ...
    (->> (cons init-op ops)
      (reduce (partial set-points fns) (transient {}))
      (persistent!)
      (vals)
      (filter true?)
      (count))))
...
#_
(let [a1 (f1 input)]
  (assert (not= a1 590985))
  (assert (= a1 569999))
  (assert (= 17836115 (f2 input)))
  (println a1))
