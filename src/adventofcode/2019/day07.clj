(ns adventofcode.2019.day07
  (:require [clojure.string :as str]
            ...
            [adventofcode.2019.day02 :as d2]
            [adventofcode.2019.day05 :as d5]))
...
(defn init-amps [phases instructions]
  (loop [[phase & todo] phases
    ...
    (if (nil? phase)
      amps
      (let [amp (d5/step {::d5/id phase ::d5/state instructions ::d5/idx 0 ::d5/input [phase input]})]
...
(assert (= 43210 (linear-mode [4,3,2,1,0] (d2/parse "3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0"))))
  ...
  (let [amps (init-amps phases instructions)]
    (loop [amps   amps
      ...
      (if (empty? amps)
        ...
        (let [x (-> amps first (assoc ::d5/input [input]) d5/step)]
          ;(prn x input)
          (if (= ::d5/halt x)
            exit
...
(assert (= 139629729
          (loop-mode [9,8,7,6,5] (d2/parse "3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5"))))
(assert (= 18216
          (loop-mode [9,7,8,5,6] (d2/parse "3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,-5,54,1105,1,12,1,53,54,53,1008,54,0,55,1001,55,1,55,2,53,55,53,4,53,1001,56,-1,56,1005,56,6,99,0,0,0,0,10"))))


(defn f [mode phases input]
  (let [instructions (d2/parse input)]
    (->> phases
      (combo/permutations)
