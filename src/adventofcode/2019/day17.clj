...
(defn make-db [input]
  (loop [[c & todo] (decode input)
         x    0
         y    0
         db   {}]
    (case c
      nil db
      \. (recur todo (inc x) y db)
      \# (recur todo (inc x) y (assoc db [x y] true))
      \^ (recur todo (inc x) y (assoc db [x y] true :bot [x y :up]))
      \> (recur todo (inc x) y (assoc db [x y] true :bot [x y :right]))
      \< (recur todo (inc x) y (assoc db [x y] true :bot [x y :left]))
      \v (recur todo (inc x) y (assoc db [x y] true :bot [x y :down]))
      \X (recur todo (inc x) y (assoc db :bot [x y :down]))
      \newline (recur todo 0 (inc y) db))))
...
(defn intersection? [db xy]
  (every? db (neighbours xy)))
...
(defn f1 [input]
  (let [db (make-db input)]
    (->> db
      (keys)
      (remove keyword?)
      (filter (partial intersection? db))
      (map score)
      (reduce +))))

(assert (= 4800 (f1 input)))

(-> input decode (str/join))
"..............#########..........................
 ..............#.......#..........................
 ..............#.......#..........................
 ..............#.......#..........................
...
(def manual-path [\L 12 \L 12 \L 6 \L 6 \R 8 \R 4 \L 12 \L 12 \L 12 \L 6 \L 6 \L 12 \L 6 \R 12 \R 8 \R 8 \R 4 \L 12 \L 12 \L 12 \L 6 \L 6 \L 12 \L 6 \R 12 \R 8 \R 8 \R 4 \L 12 \L 12 \L 12 \L 6 \L 6 \L 12 \L 6 \R 12 \R 8])

;;too long:
; A  L12 L12 L6 L6
; B  R8 R4 L12 L12 L12 L6 L6 L12 L6 R12 R8
; B  R8 R4 L12 L12 L12 L6 L6 L12 L6 R12 R8
; B  R8 R4 L12 L12 L12 L6 L6 L12 L6 R12 R8
...
(defn encode-one [v]
  (let [s (str (str/join "," v) "\n")]
    (assert (short-enough? s))
    (mapv int s)))

(assert (= (encode-one [\A,\B,\C,\B,\A,\C]) [65, 44, 66, 44, 67, 44, 66, 44, 65, 44, 67, 10]))

(defn encode [vs]
  (mapcat encode-one vs))

(defn enable-bot [cpu]
  (assoc-in cpu [::cpu/state 0] 2))
...
(defn f2 [input]
  (let [A      [\L 12 \L 12 \L 6 \L 6]
        ...
        video? [\n]
        inputs (encode [main A B C video?])
        cpu    (-> input make enable-bot (assoc ::cpu/input inputs))]
    (->> cpu
      (iterate cpu/step)
      (take-while #(not= % ::cpu/halt))
      (map ::cpu/output)
...
(assert (= 982279 (f2 input)))




(defn next-tdxy [db d xy]
  (let [sxy (u/next-xy d xy)]
    (if (db sxy)
      [nil d sxy]
      (let [ld (u/turn-left d)
            lxy (u/next-xy ld xy)]
        (if (db lxy)
          [\L ld lxy]
          (let [rd (u/turn-right d)
                rxy (u/next-xy rd xy)]
            (when (db rxy)
              [\R rd rxy])))))))
...
(defn shortest-path [db]
  (let [[x y dir] (:bot db)
        xy     [x y]
        db     (dissoc db :bot xy)
        inters (into #{} ;;to walk intersections twice
                 (filter (partial intersection? db))
                 (keys db))]
    (loop [db    db
           intrs inters
           ...
           path  []]
      (if (empty? db)
        path ;;fixme drop only if not turn;
        (if-let [[t d xy] (next-tdxy db d xy)]
          (let [i?    (contains? intrs xy)
                db    (if i? db (dissoc db xy))
                intrs (disj inters xy)]
            (if t
              (recur db intrs d xy 1 (conj path steps t))
              (recur db intrs d xy (inc steps) path)))
          (cond-> path
            :always (conj steps)
            (-> path first zero?) (next)))))))


(assert (-> input make-db shortest-path (= manual-path)))
