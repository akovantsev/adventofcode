...
(defn step [{::cpu/keys [input state idx id relbase]}]
  (loop [rb    (or relbase 0)
    ...
    (let [ins (state idx)]
      (if (cpu/exit? ins)
        ::cpu/halt
        (let [[mc mb ma _ op] (cpu/ins-vec ins)
              idx* (-> op cpu/WIDTH (+ idx))
              a    (-> idx (+ 1) (state 0))
              b    (-> idx (+ 2) (state 0))
              c    (-> idx (+ 3) (state 0))
              ai   (cpu/writev ma rb a)
              ci   (cpu/writev mc rb c)
              A    (cpu/readv state ma rb a)
              B    (cpu/readv state mb rb b)]
          (case op
            1 (recur rb input idx* (assoc state ci (+ A B)))
            2 (recur rb input idx* (assoc state ci (* A B)))
            3 (if (first input)
                (recur rb (next input) idx* (assoc state ai (first input)))
                {::cpu/id      id
                 ::cpu/idx     idx*
                 ::cpu/relbase rb
                 ::cpu/state   state
                 ::cpu/waiting-input true})
            4 {::cpu/id      id
               ::cpu/idx     idx*
               ::cpu/relbase rb
               ::cpu/state   state
...
(defn tick [input cpu]
  (let [input (or input [-1])
        cpu   (assoc cpu ::cpu/input input)]
    (->> cpu
      (iterate step)
      (rest)
      (take 3))))
...
(defn f1 [input]
  (let [seed      (make input)
        addresses (range 50)
        qconj     (fnil conj (PersistentQueue/EMPTY))
        qinto     (fnil into (PersistentQueue/EMPTY))
        set-id    #(assoc %1 ::cpu/id %2)
        nodes     (map set-id (repeat seed) addresses)
        inputs    (into {}
                    (for [id addresses]
                      [id (conj (PersistentQueue/EMPTY) [id -1])]))]
    (loop [[cpu & todo] nodes
           done    []
           outputs {}
           inputs  inputs]
      (cond
        (outputs 255) (-> 255 outputs peek second)
        (nil? cpu)    (recur done [] {} (merge-with qinto inputs outputs))
        :else         (let [id         (::cpu/id cpu)
                            input      (-> id inputs peek)
                            inputs     (update inputs id pop)
                            triple     (tick input cpu)
                            [addr x y] (map ::cpu/output triple)
                            cpu        (last triple)
                            outputs    (cond-> outputs
                                         addr (update addr qconj [x y]))
                            done       (conj done cpu)]
                        (recur todo done outputs inputs))))))


(assert (= 18513 (f1 input)))


(defn f2 [input]
  (let [seed      (make input)
        addresses (range 50)
        conjq     (fnil conj (PersistentQueue/EMPTY))
        intoq     (fnil into (PersistentQueue/EMPTY))
        set-id    #(assoc %1 ::cpu/id %2)
        nodes     (map set-id (repeat seed) addresses)
        inputs    (into {}
                    (for [id addresses]
    ...
    (loop [[cpu & todo] nodes
      ...
      (if (nil? cpu)
        ;; tick complete:
        (let [inputs (merge-with intoq inputs outputs)
              idle   (->> (select-keys inputs addresses) vals (every? empty?))
              y      (second NAT)
              prev   (some-> y (= sent))]
          (if-not idle
            ;; just next tick:
            (recur done [] inputs {} NAT sent)
            (if prev
              ;; return:
              y
              ;; send NAT to address 0
              (let [inputs (cond-> inputs
                             NAT (update 0 conj NAT))]
                (recur done [] inputs {} NAT y)))))
        ;; tick is in progress:
        (let [id      (::cpu/id cpu)
              input   (-> id inputs peek)
              inputs  (update inputs id pop)
          ...
          (if (= 255 addr)
            ;; send to NAT:
            (recur todo done inputs outputs [x y] sent)
            ;; send to node:
            (let [outputs (cond-> outputs
                            addr (update addr conjq [x y]))]
              (recur todo done inputs outputs NAT sent))))))))

(assert (= 13286 (f2 input)))