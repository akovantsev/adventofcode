...
(defn f [input init-color]
  (loop [robot  (make input init-color)
         xy     [0 0]
         dir    ::up
         canvas {xy init-color}]
    (let [on-color  (get canvas xy black)
          robot     (assoc robot ::cpu/input [on-color])
          robot     (step robot)]
      (if (= robot ::cpu/halt)
        canvas
        (let [new-color (robot ::cpu/output)
              canvas    (assoc canvas xy new-color)
              robot     (step robot)]
          (if (= robot ::cpu/halt)
            canvas
            (let [turn     (robot ::cpu/output)
                  [dir xy] (move xy dir turn)]
