...
(assert (= 159 (closest sample1)))
(assert (= 135 (closest sample2)))
...
(defn fewest [input]
  (let [sxys (->> input parse (map get-coords))
        ixys (reduce intersections sxys)]
    (->> ixys
      (map #(total-steps-count sxys %))
      (reduce min))))


(assert (= 610 (fewest sample1)))
(assert (= 410 (fewest sample2)))
(assert (= 32132 (fewest input)))